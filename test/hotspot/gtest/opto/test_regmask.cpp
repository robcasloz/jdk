...
TEST_VM(RegMask, external_member) {
  ...
  ASSERT_FALSE(rm.Member(OptoReg::Name(rm.rm_size_bits()), false));
  ASSERT_FALSE(rm.Member(OptoReg::Name(rm.rm_size_bits()), true));
  rm.set_AllStack(true);
  ASSERT_FALSE(rm.Member(OptoReg::Name(rm.rm_size_bits()), false));
  ASSERT_TRUE(rm.Member(OptoReg::Name(rm.rm_size_bits()), true));
}
...
TEST_VM(RegMask, up_extended) {
  ...
  //ASSERT_FALSE(rm.is_UP()); // FIXME: This fails unexpectedly
  rm.Clear();
  rm.set_AllStack(true);
