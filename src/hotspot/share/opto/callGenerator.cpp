...
void CallGenerator::do_late_inline_helper() {
  ...
  if (is_pure_call() && result_not_used) {
    ...
    if (C->inlining_incrementally() && !result->is_top()) {
      Node* result_proj = call->proj_out_or_null(TypeFunc::Parms);
      if (result_proj != nullptr) {
        const Type* method_type = result_proj->Value(&kit.gvn());
        const Type* result_type = result->Value(&kit.gvn());
        if (method_type != result_type && result_type->higher_equal(method_type)) {
          for (DUIterator_Fast imax, i = result_proj->fast_outs(imax); i < imax; i++) {
            if (result_proj->fast_out(i)->is_AddP()) {
