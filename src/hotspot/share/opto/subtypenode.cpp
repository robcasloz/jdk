...
Node *SubTypeCheckNode::Ideal(PhaseGVN* phase, bool can_reshape) {
  ...
  if (addr != nullptr) {
    ...
    if (!UseNewCode && con == oopDesc::klass_offset_in_bytes() && obj != nullptr) {
...
Node* SubTypeCheckNode::load_klass(PhaseGVN* phase) const {
  ...
  if (sub_t->isa_oopptr()) {
    if (UseNewCode) {
      subklass = phase->transform(LoadKlassNode::make(*phase, nullptr, phase->C->immutable_memory(), obj_or_subklass, TypeInstPtr::BOTTOM, TypeInstKlassPtr::OBJECT));
    } else {
