...
void Parse::array_store_check() {
  ...
  Node* array_klass;
  if (UseNewCode) {
    array_klass = _gvn.transform(LoadKlassNode::make(_gvn, nullptr, immutable_memory(), ary, TypeInstPtr::BOTTOM, TypeInstKlassPtr::OBJECT));
  } else {
    int klass_offset = oopDesc::klass_offset_in_bytes();
    Node* p = basic_plus_adr( ary, ary, klass_offset );
    // p's type is array-of-OOPS plus klass_offset
    array_klass = _gvn.transform(LoadKlassNode::make(_gvn, nullptr, immutable_memory(), p, TypeInstPtr::KLASS));
  }
  // Get the array klass
