...
bool G1BarrierSetC2::is_gc_barrier_node(Node* node) const {
  ...
  bool is_barrier_call = strcmp(call->_name, "write_ref_field_pre_entry") == 0 || strcmp(call->_name, "write_ref_field_post_entry") == 0;
  assert(!is_barrier_call, "unreachable");
  return is_barrier_call;
}
...
bool G1BarrierSetC2::is_gc_pre_barrier_node(Node *node) const {
  bool is_pre_val_load = is_g1_pre_val_load(node);
  assert(!is_pre_val_load, "dead code in late barrier expansion model");
  return is_pre_val_load;
}
